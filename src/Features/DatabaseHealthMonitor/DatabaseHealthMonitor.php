        ...
        $this->checks[] = function (string $table) {
            if (!$this->hasPrimaryKey($table)) {
                return [
                    ...
                    'severity' => 'critical',
                    'message' => "Tabela '{$table}' não possui chave primária",
                    'suggestion' => "Adicione uma coluna 'id' auto-increment ou UUID como primary key",
                ];
            }
            return null;
        };
        ...
        $this->checks[] = function (string $table) {
            ...
            if (!empty($duplicates)) {
                return [
                    ...
                    'severity' => 'warning',
                    'message' => "Índices duplicados encontrados em '{$table}'",
                    'details' => $duplicates,
                    'suggestion' => 'Remova índices redundantes para economizar espaço e melhorar performance de INSERT/UPDATE',
                ];
            }
            return null;
        };
            ...
            if ($rowCount > 1000000) { // 1M+ rows
                ...
                if (!empty($jsonColumns)) {
                    return [
                        ...
                        'severity' => 'warning',
                        'message' => "Tabela '{$table}' com {$rowCount} rows usa JSON",
                        'details' => ['columns' => $jsonColumns, 'rows' => $rowCount],
                        'suggestion' => 'Considere normalizar para colunas separadas ou usar TEXT para melhor performance',
                    ];
                }
            }
            return null;
        ...
        $this->checks[] = function (string $table) {
            ...
            if (!empty($unindexedFk)) {
                return [
                    ...
                    'severity' => 'warning',
                    'message' => "Chaves estrangeiras sem índice em '{$table}'",
                    'details' => $unindexedFk,
                    'suggestion' => 'Adicione índices nas colunas de FK para melhorar JOINs e evita deadlocks',
                ];
            }
            return null;
        };
            ...
            foreach ($enums as $column => $values) {
                if (count($values) > 20) {
                    return [
                        ...
                        'type' => 'large_enum',
                        'severity' => 'warning',
                        'message' => "Coluna ENUM '{$column}' em '{$table}' tem muitos valores (" . count($values) . ")",
                        'suggestion' => 'Considere usar uma tabela de lookup em vez de ENUM para flexibilidade',
                    ];
                }
            }
            return null;
        ...
        $this->checks[] = function (string $table) {
            if (!$this->hasTimestampColumns($table)) {
                return [
                    ...
                    'severity' => 'info',
                    'message' => "Tabela '{$table}' sem colunas de timestamp",
        ...
        $this->checks[] = function (string $table) {
            ...
            if (!empty($nullableNoDefault)) {
                return [
                    ...
                    'severity' => 'info',
                    'message' => "Colunas nullable sem valor default em '{$table}'",
                    'details' => $nullableNoDefault,
                    'suggestion' => 'Adicione valores default ou torne NOT NULL para evitar ambiguidade',
                ];
            }
            return null;
        };
        ...
        $this->checks[] = function (string $table) {
            ...
            if (!empty($unusedIndexes)) {
                return [
                    ...
                    'type' => 'unused_indexes',
                    'severity' => 'info',
                    'message' => "Índices não utilizados em '{$table}'",
                    'details' => $unusedIndexes,
                    'suggestion' => 'Considere remover índices não utilizados para economizar espaço',
                ];
            }
            return null;
        };
        ...
        $this->checks[] = function (string $table) {
            ...
            if ($autoIncrementInfo && $autoIncrementInfo['percentage'] > 80) {
                return [
                    ...
                    'severity' => 'critical',
                    'message' => "Auto increment de '{$table}' próximo do limite ({$autoIncrementInfo['percentage']}%)",
                    'details' => $autoIncrementInfo,
                    'suggestion' => 'Migre para BIGINT UNSIGNED ou considere arquivar dados antigos',
                ];
            }
            return null;
        };
        ...
        $this->checks[] = function (string $table) {
            ...
            if (!empty($largeStringIndexes)) {
                return [
                    ...
                    'severity' => 'warning',
                    'message' => "Índices em colunas VARCHAR muito grandes em '{$table}'",
                    'details' => $largeStringIndexes,
                    'suggestion' => 'Use prefixo de índice: $table->index(DB::raw("email(191)")) ou reduza o tamanho',
                ];
            }
            return null;
        };
